...
if [ $stage -le 15 ]; then
  ...
  if [ -d $lang ]; then
    if [ $lang/L.fst -nt $lang/L.fst ]; then
...
if [ $stage -le 17 ]; then
  ...
  relu-batchnorm-dropout-layer name=tdnn-target input=Append(tdnn11.batchnorm@-3,tdnn11.batchnorm) dim=1280
