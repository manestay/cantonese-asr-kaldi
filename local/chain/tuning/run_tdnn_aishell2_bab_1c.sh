...
src_mfcc_config=../../aishell2/s5/conf/mfcc_hires.conf # mfcc config used to extract higher dim
                                                  ...
                                                  # in source domain.
src_ivec_extractor_dir=../../aishell2/s5/exp/chain/extractor_all  # Source ivector extractor 
...
src_lang=../../aishell2/s5/data/lang # source lang directory used to train source model.
...
src_dict=../../aishell2/s5/data/local/dict  # dictionary for source dataset containing lexicon.txt,
                                            # nonsilence_phones.txt,...
                                            # lexicon.txt used to generate lexicon.txt for
                                            # src-to-tgt transfer.

src_tree_dir=../../aishell2/s5/exp/chain/tri4_cd_tree_sp # chain tree-dir for src data;
...
# lang_src_tgt=data/lang_aishell2_bab # This dir is prepared using phones.txt and lexicon from
                              # source(WSJ) and and wordlist and G.fst from target(RM)
lang_src_tgt=data/lang # default dir
...
ivec_opt=""
if $use_ivector;then ivec_opt="--online-ivector-dir ../../aishell2/s5/exp/chain/ivectors_train_all" ; fi
...
if [ $stage -le 14 ]; then
  ...
  steps/align_fmllr_lats.sh --nj $nj --cmd "$train_cmd" data/train \
    data/lang/ exp/tri4 exp/tri4_lats || exit 1;
  rm exp/tri4_lats/fsts.*.gz 2>/dev/null || true # save space
  ...
  # steps/nnet3/align_lats.sh --nj 100 --cmd "$train_cmd" $ivec_opt \
    ...
    # data/train_sp_hires $lang_src_tgt $src_mdl_dir $lat_dir || exit 1;
...
if [ $stage -le 18 ]; then
  ...
  # the lang directory.
  tes_ivec_opt=""
  if $use_ivector;then test_ivec_opt="--online-ivector-dir ../../aishell2/s5/exp/chain/ivectors_test_all" ; fi
